...
## **Conclusions**
The goal of this project was to deepen our understanding of prolog by coding a game.

Throughout the development, we had to reconsider how to translate previous methods from other languages into prolog. Whether they could be more efficient due to the inner workings of prolog, or if they were incompatible with the language altogether. In short, we had to change our perspective from iterative programming to declarative programming, something way easier said than done sometimes.

Still, we managed to deliver a fully working game that we are proud of. All the features proposed were implemented, and the program can deal with multiple sizes of boards and supports two levels of artificial intelligence (one random and one greedy).

There isn’t a clear path for improvements at the moment. Maybe some little things, but significant improvements don’t seem logical with the current approach. We could have devised a different way to evaluate the boards that would be a better representation of the inner logic of the game, improving our greedy AI. Although, every attempt at this ended with a time-consuming algorithm. Or perhaps we could have implemented a mid-level AI that picks a suboptimal move. However, this would only require us to choose a move further down the list that we already calculate, so there is not much reason to do that.

All in all, we are fully confident in the project we have developed, and we believe that it proves that we have deepened our understanding of prolog.
