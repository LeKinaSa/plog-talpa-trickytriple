...
* Value: An integer representative of the advantage that player Player has over is opponent. 
*        If Value > 0 then player Player is wining, if Value==0 then both players are equally developed, if Value < 0 then player Player is losing.
*        Value takes a huge negative value (~9999999) if the oppositive player of Player has connected their sides of the board (meaning Player as lost)
*        Value takes a huge positive value (~9999) if player Player has connected their sides of the board (meaning Player as won).
...
*           used to decide the next move of player Player and acording to the rules if both a move connects the side for both players then the one that made the 
